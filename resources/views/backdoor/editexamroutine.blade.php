        ...
        <div class="container-fluid"> <!-- coustom image -->
            <div class="row">
                <div class="col-md-6 m-auto">
                    <div class="card">
                        <div class="card-header"> <h3>Exam Updating </h3> </div>
                        <div class="card-body">
                            ...
                            @endif  
                            <form class="form text-danger font-weight-bold" action="{{ route('updateexamroutine') }}" method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}  
                                <input type="hidden" name="id" value="{{$examroutine->id}}">
                                <div class="form-group">
                                    <lable for="department">Department</lable>
                                    <select class="form-control" name="department">
                                        <option value="Diploma In Survey">{{$examroutine->department}}</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    <lable for="semister">Semister</lable>
                                    <select class="form-control" name="semister">
                                        <option>{{$examroutine->semister}}</option>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="exam">
                                        <option> {{$examroutine->exam}} </option>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="prostatus">
                                         <option>{{$examroutine->status}}</option>
                                ...
                                <div class="form-group text-center">
                                    <img src="{{ asset('/') }}{{$examroutine->avatar}}" style="width:150px; margin-top:5px;"/>
                                </div>
                                
                                <div class="form-group mt-2">
                                    <input type="submit" value="Submit" class="btn btn-primary">
                                    <a href="{{route('backexamroutine')}}" class="btn btn-success"> exam list</a>
                                </div>
                            </form>
                        </div>
                    </div>
